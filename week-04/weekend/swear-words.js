...
function familyFriendlizer(pa1, pa2) {
  const clearT = [];
  let removedWords = 0;
  pa1.split(' ').forEach((e) => {
    if ((pa2.includes((e.replace(/,/g, '')).toString().toLowerCase()) === true) || (pa2.includes((e.replace(/\./g, '')).toString().toLowerCase()) === true)) {
      clearT.push('');
      removedWords += 1;
    } else {
      clearT.push(e);
  ...
  const clearText = clearT.join(' ');
  return { clearText, removedWords };
}
